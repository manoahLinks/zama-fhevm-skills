...
The randomness is generated by the coprocessor and is **never revealed on-chain** — you get a ciphertext handle, same as any other encrypted value. Nobody, including the caller and the miner, sees the value unless it is later decrypted through the normal ACL + decryption flow.

```solidity
euint8  r1 = FHE.randEuint8();           // unbounded [0, 255]
euint32 r2 = FHE.randEuint32();          // unbounded [0, 2^32 - 1]
euint8  r3 = FHE.randEuint8(32);         // bounded [0, 31] — bound MUST be power of 2
...
| Unbounded | Bounded |
|---|---|
| `randEuint8()` | `randEuint8(uint8 upperBound)` |
| `randEuint16()` | `randEuint16(uint16 upperBound)` |
| `randEuint32()` | `randEuint32(uint32 upperBound)` |
| `randEuint64()` | `randEuint64(uint64 upperBound)` |
| `randEuint128()` | `randEuint128(uint128 upperBound)` |
| `randEuint256()` | `randEuint256(uint256 upperBound)` |
| `randEbool()` | — |

**Three rules you cannot break:**

1. **The bound must be a power of two**, and the range is `[0, upperBound - 1]` — exclusive at the top. `randEuint8(32)` yields `0..31`, not `0..32`. A non-power-of-2 bound does not produce a uniform distribution.
2. **`rand*` cannot be called from `eth_call`.** Only real transactions work, because the coprocessor mutates PRNG state. Any `staticCall`/`.call` simulation of a function containing `rand*` silently fails — this includes gas estimation on some tooling, and ethers `contract.fn.staticCall(...)`.
3. **The bound is plaintext.** You cannot pass an `euint` as `upperBound`.

#### Getting a range that isn't a power of two

Rejection sampling needs a loop with an encrypted condition, which is impossible. Use modulo bias-tolerant reduction, or oversample and select:

```solidity
// Want [0, 5] (a 6-sided die). 6 is not a power of 2.
// Draw from [0, 7] and clamp — slightly biased toward 5, but branchless.
euint8 raw = FHE.randEuint8(8);
ebool tooBig = FHE.gt(raw, FHE.asEuint8(5));
euint8 die = FHE.select(tooBig, FHE.asEuint8(5), raw);
```

If uniformity matters, draw a much wider value and reduce — the bias shrinks with width. There is no unbiased branchless option, so choose deliberately.

#### Common pattern: assign a secret random winner

```solidity
function drawWinner() external onlyAfterClose {
    // participantCount is plaintext; round it up to a power of 2
    euint32 idx = FHE.randEuint32(nextPowerOfTwo);
    winnerIndex = idx;

    FHE.allowThis(winnerIndex);              // contract reads it in a later tx
    FHE.makePubliclyDecryptable(winnerIndex); // or FHE.allow(...) for a private reveal
}
```

Note this is a **transaction**, not a view — see rule 2.
