...
3. Off-chain, a client calls `sdk.decryption.decryptPublicValues(...)` and gets the plaintexts plus a proof.
...
- The exact handle-order requirement for `FHE.checkSignatures` / `decryptPublicValues`.
...
The client **must** call `decryptPublicValues` with the same order:

```typescript
await sdk.decryption.decryptPublicValues([winningAddressHandle, winningBidHandle]);
```
